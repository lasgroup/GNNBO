...
plot_name = 'scalability_all_{}d'.format(configs['feat_dim'])
fig, axes = plt.subplots( nrows =4,ncols=3)
plt.rcParams.update(bundles.neurips2022(nrows =4,ncols=3, tight_layout=True))

generic_lines = ['#59adf6','#949494','#ff6961']
...
for net in ['GNN', 'NN']:
    ...
    for edge_prob in [0.05, 0.2, 0.95]:
        df_p = df_net.loc[df_net['edge_prob'] == edge_prob]
        df_p_us = df_net_us.loc[df_net_us['edge_prob'] == edge_prob]

        for color_counter, num_nodes in enumerate([5, 20, 100]):
            df_n = df_p.loc[df_p['num_nodes'] == num_nodes]
            df_n_us = df_p_us.loc[df_p_us['num_nodes'] == num_nodes]
            ...
            df_n_us = df_n_us.loc[df_n_us['alg_lambda'] == alg_lambdas[algo_us][d_counter]]
            df_n_us = df_n_us.loc[df_n_us['exploration_coef'] == alg_betas[algo_us][d_counter]]
            # def_net_us = df_net_us.loc[df_net_us['pretrain_steps'] == alg_pretrain_steps[algo_us][d_counter]]
            curve_name = r' $N = $' + str(num_nodes)
            regrets_bp = np.array([np.squeeze(np.array(regrets)) for regrets in df_n['regrets_bp']])
            regrets_bp_us = np.array([np.squeeze(np.array(regrets)) for regrets in df_n_us['regrets_bp']])
            ...
            axes[row_counter][col_counter].set_xlabel( algo_names[algo] +  r', $ p = $' + str(edge_prob))
            axes[row_counter+1][col_counter].set_xlabel(algo_names[algo_us] + r', $ p = $' + str(edge_prob))
...
lines_labels = [ax.get_legend_handles_labels() for ax in [axes[0][2]]]
